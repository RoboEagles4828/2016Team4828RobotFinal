...
public class CameraMotors {
	...
	public CameraMotors(CANTalon h, CANTalon v) {
		motorH = h;
		motorV = v;
	...
	private final static double DEGREE = .04; // base speed
		...
		if (centerX[0] > 0) {
			...
			if (centerX.length > 0) {
				...
				if (xval > CAMERA_X_CENTER + DEADZONE) {
					//set(motorH, get(motorH) + DEGREE * xmult);
					// System.out.println("too far left");
					motorH.set(DEGREE*xmult);
				...
				if (yval > CAMERA_Y_CENTER + DEADZONE) {
					//set(motorV, get(motorV) + DEGREE * ymult / 2);
					// System.out.println("too far up");
					motorV.set(DEGREE*ymult);
	...
	/*public void aimCameraMotors(RobotDrive drive) {
		...
		if (centerX.length > 0) {
			...
			if (centerX.length > 0) {
				...
				if (yval > CAMERA_Y_CENTER + DEADZONE) {
					motorV.set(motorV.get() + DEGREE * ymult / 2);
					// System.out.println("too far up");
				} else if (yval < CAMERA_Y_CENTER - DEADZONE) {
					motorV.set(motorV.get() - DEGREE * ymult / 2);
					// System.out.println("too far down");
	...
	/*public void stickAim(Joystick cameraStick) {
		...
		motorH.setAngle(hSet);
		vSet = cameraStick.getY() * -100 + 120; // Pilot Style
		// System.out.println("X " + motorH.getAngle());
		// System.out.println("Y " + motorV.getAngle());
		if (vSet > 156) {
			vSet = 156;
		}
		if (vSet < 80) {
